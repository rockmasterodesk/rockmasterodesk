$(document).ready(function(){
  ...
  window.rushModalConfirm = function(options) {
    ...
    if ("buttons" in options) {
      if (options['buttons']['ok'] == '') { rushButtonOK = 'Ok';} else { rushButtonOK = options['buttons']['ok'];}
      if (options['buttons']['cancel'] == '') { rushButtonCancel = 'Cancel';} else { rushButtonCancel = options['buttons']['cancel']; }
    } else {
      rushButtonOK = 'Ok';
    ...
    if ("closable" in options) {
      ...
      str = '<div class="modal" id="'+dialogid+'">'+
      '<div class="modal-dialog modal-dialog-centered" style="'+style+'" >'+
        '<div class="modal-content">'+
          '<div class="modal-header">'+
            '<h4 class="modal-title">'+options["title"]+
            '</h4>'+
            xclose+
          '</div>'+
    ...
    '</div>';
    $('html').append(str);
    $("#"+dialogid).modal({backdrop: backdrop, keyboard: keyboard});
